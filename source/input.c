...
int input_read_parameters(
  ...
  double Omega_tot, Omega_at_z, Omega_lambda, Omega0_rad;
  ...
  if(pba->arbitrary_species_number_of_knots > 0){
        ...
        for(i=0;i<pba->arbitrary_species_number_of_knots;i++){
          ...
          if(pba->arbitrary_species_density_at_knot[i]!= 0.0){
            ...
            Omega_at_z = (pba->Omega0_b+pba->Omega0_cdm)*pow(1+pba->arbitrary_species_redshift_at_knot[i],3)+(Omega0_rad)*pow(1+pba->arbitrary_species_redshift_at_knot[i],4)+Omega_lambda;
            pba->arbitrary_species_at_knot[i*4]=pba->arbitrary_species_fraction_at_knot[i]*Omega_at_z/(1-pba->arbitrary_species_fraction_at_knot[i]);
            if(pba->arbitrary_species_interpolation_is_log == _TRUE_){
              pba->arbitrary_species_at_knot[i*4]=log10(pba->arbitrary_species_at_knot[i*4]);
              pba->arbitrary_species_redshift_at_knot[i]=log10(pba->arbitrary_species_redshift_at_knot[i]+1);
            }
